...
public class GraficoPrevisaoRNA {
    ...
    private ModeloGraficoRNA modelo;
    private String[] anosSelecionados;
    private double[][] planilha;

    public GraficoPrevisaoRNA() {
    }

    public GraficoPrevisaoRNA(String tituloGrafico, String tituloEixoX, String tituloEixoY,
        ...
        lista = new ArrayList<ModeloGraficoRNA>();
        try {
    ...
    public GraficoPrevisaoRNA(String tituloGrafico, String tituloEixoX,
...
//        System.out.println("periodo.size(): " + periodo.size());
//        System.out.println("previsto.size(): " + previsto.size());
        // DADOS PREVISTOS
        for (int i = 0; i < previsto.size(); i++) {
            lista.add(new ModeloGraficoRNA(formataMes(periodo.get(i).split("/")[0]), String.valueOf(Math.abs(Double.valueOf(
                    previsto.get(i).replace(".", "").replace(",", ".")))), periodo.get(i).split("/")[1] + " - Previsto"));
        }
        ...
        if (anosSelecionados != null) {
            for (int i = 0; i < anosSelecionados.length; i++) {
                for (int j = 0; j < planilha.length; j++) {
                    lista.add(new ModeloGraficoRNA(formataMes(String.valueOf(j)), String.valueOf(planilha[j][i]), anosSelecionados[i] + " - Histórico"));
                }
            }
        }
...
//                lista.add(new ModeloGraficoRNA(formataMes(periodo.get(i).split("/")[0]), historico.get(i), periodo.get(i).split("/")[1] + " - Histórico"));
//            } else {
//                break;
//            }
//        }
...
//        }
    ...
    public static BufferedImage gerarGraficoLinha(String tituloGrafico, String tituloEixoX,
            String tituloEixoY, ArrayList<ModeloGraficoRNA> arrayValores) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                ModeloGraficoRNA modelo = (ModeloGraficoRNA) iterator.next();
                defaultCategoryDataset.addValue(Math.abs(Double.valueOf(modelo.getValor().replace(".", "").replace(",", ".")).doubleValue()),
