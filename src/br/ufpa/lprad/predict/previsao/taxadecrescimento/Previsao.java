...
import br.ufpa.lprad.predict.previsao.simples.*;
...
//            Previsao2Anos p = new Previsao2Anos();
            PrevisaoTaxadeCrescimento ptc = new PrevisaoTaxadeCrescimento();
//            p.leArq();
//            p.definePeriodo();
//            p.preveTodosAnos();
...
//            p.formataRelatorio();
            ...
            ThreadGeraRelProjecao geraRelProjecao = new ThreadGeraRelProjecao();
...
//
//                new Grafico(p.getPeriodo(),/* p.getDadosHistoricos()*/ converteDados(Previsao2Anos.getDadosRelatorio(), 1),/* p.getDadosPrevistos()*/ converteDados(Previsao2Anos.getDadosRelatorio(), 2), p.getErroTotal(), p.getErroMensal());
//            }
//            if (((PrevisaoBean.getAnoFim() - PrevisaoBean.getAnoIni()) == 2) || ((PrevisaoBean.getAnoFim() - PrevisaoBean.getAnoIni()) == 1) || (difMeses >= 1)) {
//
...
//            }
    ...
    private String[] converteDados(String[][] dadosRelatorio, int indice) {
        String[] vetor = new String[dadosRelatorio.length];

        int aux = 0;
        for (int i = 0; i < vetor.length; i++) {
            if (!dadosRelatorio[i][indice].trim().equalsIgnoreCase("-")) {
                vetor[aux] = dadosRelatorio[i][indice];
                aux++;
            }
        }
        int numValidos = 0;
        ...
        String result[] = new String[numValidos];
