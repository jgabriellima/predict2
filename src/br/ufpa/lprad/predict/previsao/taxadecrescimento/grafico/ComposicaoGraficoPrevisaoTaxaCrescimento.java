 ...
 * @author gabriel
 */
public class ComposicaoGraficoPrevisaoTaxaCrescimento {

    private List<ModeloGraficoTaxaCrescimento> lista;
    private String[][] matriz;
    private Vector anos = new Vector();
    private String ano;

    public ComposicaoGraficoPrevisaoTaxaCrescimento(String[][] matriz) {
        this.matriz = matriz;
            ...
            if (!matriz[i][0].equalsIgnoreCase("TOTAL")) {
                if (!anos.contains(matriz[i][0].split("/")[1])) {
                    ...
                    if (!matriz[i][2].equalsIgnoreCase("inexistente") && !matriz[i][2].isEmpty()) {
                        lista.add(new ModeloGraficoTaxaCrescimento(formataMes(matriz[i][0].split("/")[0]), matriz[i][2], ano));
                    }
                }
            }
        }
