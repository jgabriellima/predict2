 ...
 * @author gabriel
 */
public class GraficoPrevisaoTaxaDeCrescimento {
    ...
    public GraficoPrevisaoTaxaDeCrescimento() {
    }

    public GraficoPrevisaoTaxaDeCrescimento(String tituloGrafico, String tituloEixoX,
        ...
        lista = new ArrayList<ModeloGraficoTaxaCrescimento>();
        try {
    ...
    private void criaLista() {
        ...
        for (int i = 0; i < previsto.size(); i++) {
            lista.add(new ModeloGraficoTaxaCrescimento(formataMes(periodo.get(i).split("/")[0]), previsto.get(i), periodo.get(i).split("/")[1] + " - Previsto"));
        }
//        // DADOS HISTORICO - PREVISTO
//        for (int i = 0; i < historico.size(); i++) {
//            if (!historico.get(i).equalsIgnoreCase(naoExisteMsg)) {
//                lista.add(new ModeloGraficoTaxaCrescimento(formataMes(periodo.get(i).split("/")[0]), historico.get(i), periodo.get(i).split("/")[1] + " - Histórico"));
//            } else {
//                break;
//            }
//        }
        ...
        if (anosSelecionados != null) {
            for (int i = 0; i < anosSelecionados.length; i++) {
                for (int j = 0; j < planilha.length; j++) {
                    lista.add(new ModeloGraficoTaxaCrescimento(formataMes(String.valueOf(j)), String.valueOf(planilha[j][i]), anosSelecionados[i] + " - Histórico"));
                }
            }
        }
...
//        }
    ...
    public static BufferedImage gerarGraficoLinha(String tituloGrafico, String tituloEixoX,
            String tituloEixoY, ArrayList<ModeloGraficoTaxaCrescimento> arrayValores) {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                ModeloGraficoTaxaCrescimento modelo = (ModeloGraficoTaxaCrescimento) iterator.next();
