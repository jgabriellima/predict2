...
import br.ufpa.lprad.predict.previsao.simples.Grafico;
import br.ufpa.lprad.predict.previsao.simples.Previsao2Anos;
 ...
 */
public class RelHistoricoFrame extends javax.swing.JInternalFrame {
    ...
    public RelHistoricoFrame() {
        setTitle("Relatório de Projeção  -  Análise da Taxa de Crescimento : " + PrevisaoBean.getNomeConsumo());
        initComponents();
        ...
        //----------------------------------------------------
        valor = PrevisaoTaxadeCrescimento.getRelatorioSimples();//Previsao2Anos.getDadosRelatorio();
...
//        fazPrevisao();
        GerenciadorDesktop.add(
                new EscolherAnosGraficoTaxaCrescimento("Grafico de Previsão \n" + PrevisaoBean.getTipoConsumo() + "\n" + PrevisaoBean.getNomeConsumo(), "Período", "Valor", PrevisaoTaxadeCrescimento.periodoPrevisto, PrevisaoTaxadeCrescimento.dadoPrevisto, PrevisaoTaxadeCrescimento.dadosHistoricosPrevisao, PrevisaoTaxadeCrescimento.getErroGeral()));

//        new GraficoPrevisaoTaxaDeCrescimento("Grafico de Previsão \n" + PrevisaoBean.getTipoConsumo() + "\n" + PrevisaoBean.getNomeConsumo(), "Período", "Valor", PrevisaoTaxadeCrescimento.periodoPrevisto, PrevisaoTaxadeCrescimento.dadoPrevisto, PrevisaoTaxadeCrescimento.dadosHistoricosPrevisao, PrevisaoTaxadeCrescimento.getErroGeral());

    }//GEN-LAST:event_jButton4ActionPerformed

    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed

        if (!PrevisaoTaxadeCrescimento.getErroGeral().equalsIgnoreCase("inexistente")) {

            try {
                ComposicaoGraficoErro cp = new ComposicaoGraficoErro(PrevisaoTaxadeCrescimento.getRelatorioSimples());
                GeradorGrafico.gerarGraficoBarraVertical3D("Gráfico de Erros", "Período", "Valor", cp.getLista(), "Erro Percentual: " + PrevisaoTaxadeCrescimento.getErroGeral() + "%  ");
            } catch (Exception ex) {
                ex.printStackTrace();
            }
    ...
    public void fazPrevisao() {
        try {
            Previsao2Anos p = new Previsao2Anos();
            p.leArq();
            p.definePeriodo();
            p.preveTodosAnos();
            p.defineValoresPrevistos();
            p.defErro();
            p.calcErroAnual();
            p.mediaErro();
            p.formataRelatorio();
            if (((PrevisaoBean.getAnoFim() - PrevisaoBean.getAnoIni()) == 2) || ((PrevisaoBean.getAnoFim() - PrevisaoBean.getAnoIni()) == 1) || (difMeses >= 1)) {
                new Grafico(p.getPeriodo(), p.getDadosHistoricos(), p.getDadosPrevistos(), p.getErroTotal(), p.getErroMensal());
            }
